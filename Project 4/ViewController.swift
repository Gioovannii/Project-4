...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    // method if page should be load or not
    // escaping key word that so closure can escape the current method
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        // set url to the URL of navigation
        let url = navigationAction.request.url
        
        // 1. Unwrapped value of url.host
        // 2. Loop over websites array
        // 3. check if each safe website exist in the host name
        // 4. If all ok we allow loading
